...
seqs <- joined %>% filter(tdr_left_start < 1000) %>% 
          filter(TDR_in != 0) %>% 
          filter(u_length_ < 11000) %>% select(chrm)
...
flips <- as.vector(unique(g0[g0$seq_id %in% sequences$seq_id, ] %>% 
                            filter(strand == '-') %>% select(seq_id)))$seq_id

genes <- g0 %>% filter(seq_id %in% sequences$seq_id)


p <- gggenomes(seqs=sequences, genes=genes, feats = f0) %>%
  flip(all_of(flips)) + # %>%
  #focus(.track_id = genes, .expand = 100, .overhang='drop') + # %>% 
  ...
  # geom_gene_text(aes(label=product), size=8) +  # add gene cluster
...
ggsave('pics/have_tdrs.pdf', dpi=600,  height = 0.7 * nrow(sequences), width = 40, limitsize = FALSE)
