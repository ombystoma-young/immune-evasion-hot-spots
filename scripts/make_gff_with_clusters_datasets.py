...
def filter_later_genes(path_long: str, path_wide: str, path_upstream: str):
...
if __name__ == '__main__':
    wc = parse_args().wildcard

    res_dir = 'results'
    prefix = f'upstream_proteins_{wc}_clu'
    ...
    upstream_file = os.path.join(upstream_dir, f'upstream_{wc}.gff')
    upstream_mod_file = os.path.join(res_dir, f'upstreams_with_clusters_{wc}.gff')
    modified_upstream = filter_later_genes(path_long_results, path_wide_results, upstream_file)
