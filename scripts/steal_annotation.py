...
def read_double_gff(path: str) -> dict:
    couples = {}
    with open(path, 'r') as d_gff_file:
        for line in d_gff_file:
            entry = line.strip().split('\t')
            if entry[2] == 'CDS':
                stealed_attribute = {el.split('=')[0]: el.split('=')[1] for el in entry[8].split(';')}
                our_attribute = {el.split('=')[0]: el.split('=')[1] for el in entry[17].split(';')}
                couples[our_attribute['ID']] = stealed_attribute['product']
    return couples
...
def assign_clusters_products(path_clus: str, functions: dict, function_additional : dict, long_file_name: str) -> dict:
    ...
    with open(long_file_name, 'w') as long_f:
        with open(path_clus, 'r') as clust_proteins:
            for line in clust_proteins:
                ...
                clu_parent = couple[0]
                clu_child = couple[1]
                if clu_child not in functions.keys():
                    clu_child_func = function_additional[clu_child]
                else:
                    clu_child_func = functions[clu_child]
...
if __name__ == '__main__':
    in_gff = os.path.join('upstream_search', 'meta_upstream.gff')
    ...
    clus_out_long = os.path.join('results', 'upstream_proteins_clu_long.tsv')
    clus_out_wide = os.path.join('results', 'upstream_proteins_clu_wide.tsv')
    clus_out_wide_seq = os.path.join('results', 'upstream_proteins_clu_wide_seq.tsv')

    func_l = read_double_gff(in_gff)
    func_prokka = read_prokka_gff(our_gff)
    faa_dict = read_faa(clus_faa)

    clu_wide = assign_clusters_products(clus, func_l, func_prokka, clus_out_long)
